...
void DFS(double **adj, int n, int start, bool *visited, int *path, int *final_path, double *final_res, double curr_weight, int level);
...
void DFS(double **adj, int n, int start, bool *visited, int *path, int *final_path, double *final_res, double curr_weight, int level) {
    ...
    if (level == n - 1) {
        if (adj[path[level]][path[0]] != 0) {
            double curr_res = curr_weight + adj[path[level]][path[0]];

            if (curr_res < *final_res) {
                *final_res = curr_res;
                memcpy(final_path, path, (n + 1) * sizeof(int));
                final_path[n] = path[0];
            }
        }
    } else {
        for (int i = 0; i < n; i++) {
            if (!visited[i] && adj[start][i] != 0) {
                ...
                if (temp_weight < *final_res) {
                    DFS(adj, n, i, visited, path, final_path, final_res, temp_weight, level + 1);
                }
            }
        }
    }
...
void depthFirstSearchAlgorithm(City cities[], int totalCities, int startLocation) {
    ...
    clock_t start = clock();
    DFS(adj, totalCities, startLocation, visited, path, final_path, &final_res, 0, 0);
    clock_t end = clock();
    double timeElapsed = (double)(end - start) / CLOCKS_PER_SEC;
